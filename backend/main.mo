...
actor {
  ...
  var heroSection : CommonSection = {
    sectionTitle = "Welcome to Aparna Millets";
    content = "Join the Millet Revolution for sustainable food processing. At Aparna Millets, we are pioneering tasty distributes of healthcare-certified millet products and supporting smallholders with sustainable trade, contract farming, and value-adding processing. Marketplace innovation, sustainable cash crop support, chemical-free cultivation, and healthcare-certified millet products.";
    images = [];
  };
  ...
  // Admin-only: Get unread messages count
  public query ({ caller }) func getUnreadMessagesCount() : async Nat {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can view message counts");
    };
    messageState.size();
  };
};
